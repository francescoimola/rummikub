...
If we must discuss the human element, rest assured, it is integral to my work, whether or not it can be detected from a first glance. If AI has a strength is that it's really, really good at coding. In fact, that's exactly where I've taken advantage of it most. I did not, however, let it touch my designs (beyond fixing the inevitable bugs) because I wanted total control over UI, UX, and how the user would interact with the website, which is ultimately both a portfolio and an extension of my identity. 
