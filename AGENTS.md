...
| `src/css/_colors.scss` | Figma-generated colour scales (brand/green, neutral, success, warning, error, info) with light-dark() for dark mode (`@layer colors`) |
...
7. **Colour scales** — All colour scales (brand/green, neutral, success, warning, error, info) use Figma-generated oklch values with `light-dark()` for dark mode. Scales use 50–950 step numbering. Neutral has both solid (`--neutral-{step}`) and alpha (`--neutral-{step}-alpha`) variants. Alpha variants use the darkest neutral step as base (950 in light, 50 in dark) with consistent alpha values across modes.
