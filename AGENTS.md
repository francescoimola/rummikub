...
7. **Neutral colour tokens** — `--neutral-1` through `--neutral-6` are theme-independent tinted alpha values generated by a power curve in `_colors.scss`. They use `light-dark()` for dark mode. `--neutral-1` = headings/primary text, `--neutral-2` = body text, `--neutral-3` = `small`/`figcaption`/`blockquote`/`time`, `--neutral-4` = medium-strength UI, `--neutral-5` = faint borders/`hr`, `--neutral-6` = image backgrounds. Tuning knobs: `$neutral-min` (alpha floor, default `0.08`) and `$neutral-gamma` (curve exponent, default `0.70`).
